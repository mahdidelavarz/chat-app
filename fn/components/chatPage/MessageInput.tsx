...
export default function MessageInput({ onSendMessage, onTypingStart }: MessageInputProps) {
  ...
  useEffect(() => {
    if (newMessage) {
      const hasPersian = isPersianText(newMessage);
      setTextDirection(hasPersian ? 'rtl' : 'ltr');
    } else {
  ...
  const hasPersian = isPersianText(newMessage);
  const placeholder = hasPersian ? "پیام خود را بنویسید..." : "Type a message...";
  ...
  return (
    <div className="bg-gradient-to-r from-[#1D2733] to-[#222E3C] border-t border-gray-700/50 p-2 sm:p-3 md:p-4 shrink-0 safe-area-bottom">
      <form onSubmit={handleSubmit} className="flex items-center gap-1.5 sm:gap-2">
        ...
        >
          <SolarArrowDown width={20} height={20} className={`text-white sm:w-6 sm:h-6 ${hasPersian ? 'rotate-90' : 'rotate-180'}`} />
        </button>
      </form>
    </div>
  );
